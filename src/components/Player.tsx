...
export function Player({ onLock, onUnlock, prayerActive }: { onLock?: () => void, onUnlock?: () => void, prayerActive?: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (isMobile) {
      euler.current.setFromQuaternion(camera.quaternion);
      euler.current.y -= inputState.lookX * 0.0006; // Lowered from 0.0012 for smoother mobile touch look
      euler.current.x -= inputState.lookY * 0.0006; // Lowered from 0.0012 for smoother mobile touch look
      euler.current.x = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, euler.current.x));
      camera.quaternion.setFromEuler(euler.current);
      inputState.lookX = 0;
      inputState.lookY = 0;
