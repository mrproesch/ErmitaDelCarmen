...
function AltarPainting() {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (tex) => {
        ...
        setAltarTexture(tex);
        console.log(`Successfully loaded altar texture from: ${relativePath}`);
      },
      undefined,
      () => {
        if (relativePath !== '/altar.jpg') {
          loader.load(
            '/altar.jpg',
            (tex) => {
              if (!isMounted) return;
              ...
              tex.needsUpdate = true;
              setAltarTexture(tex);
              console.log('Successfully loaded altar texture from root path');
            },
            undefined,
            () => {
              console.log('Using default warm placeholder for altar painting.');
            }
          );
        } else {
          console.log('Using default warm placeholder for altar painting.');
  ...
  return (
    <group position={[0, 4.3, -7.65]}>
      {/* A simple landscape oriented rectangle that loads the texture directly */}
      <mesh key={altarTexture ? 'loaded' : 'loading'} castShadow receiveShadow>
