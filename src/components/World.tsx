...
function AltarPainting() {
  ...
  useEffect(() => {
    ...
    const imagePath = `${cleanBase}altar.jpg`;

    const handleTextureLoaded = (tex: THREE.Texture) => {
      ...
      setAltarTexture(tex);
      console.log(`Successfully loaded static altar picture from ${imagePath}`);
    };
    ...
    loader.load(
      ...
      undefined,
      (err) => {
        console.warn('Error loading altar.jpg with base path, trying fallback...', err);
        loader.load(
          'altar.jpg',
          handleTextureLoaded,
          undefined,
          (fallbackErr) => {
            console.error('Failed to load altar.jpg texture', fallbackErr);
          }
        );
      }
    );
