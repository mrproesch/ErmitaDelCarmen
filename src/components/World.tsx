...
function BrickPath() {
  ...
  useLayoutEffect(() => {
    ...
    bricks.forEach((brick, i) => {
      dummy.position.set(brick.x, 0.02, brick.z);
...
function Squirrel() {
  ...
  const pos = useRef(new THREE.Vector3((Math.random() - 0.5) * 40, 0.1, (Math.random() - 0.5) * 40));
  ...
  useFrame((_, delta) => {
    ...
    if (state.current.phase === 'idle') {
      ...
      if (dist < 0.5) {
        ...
        pos.current.add(dir.multiplyScalar(4 * delta));
        groupRef.current.position.copy(pos.current);
  ...
  return (
    <group>
      ...
      {streetLights.map((sl, i) => (
        <StreetLight key={`sl-${i}`} position={sl.pos as [number, number, number]} rotation={sl.rot as [number, number, number]} />
      ))}
      {benches.map((b, i) => (
        <Bench key={i} position={b.pos as [number, number, number]} rotation={b.rot as [number, number, number]} />
      ))}
