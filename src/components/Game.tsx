...
export default function Game() {
  ...
  return (
    <div className="w-full h-screen bg-black overflow-hidden relative touch-none select-none">
      ...
      {!isLocked && !isMobile && !prayerOpen && (
        <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black/60 z-10">
          <div className="text-white text-center p-8 bg-gray-900/80 rounded-xl backdrop-blur-sm border border-white/10">
            ...
            <p className="text-lg mb-2">Haz clic en cualquier lugar para empezar</p>
            <div className="text-sm text-gray-400 space-y-1 mt-4">
              <p>WASD para moverse</p>
              <p>Ratón para mirar</p>
              <p>P para leer Oraciones</p>
              <p>ESC para pausar</p>
            </div>
          </div>
        </div>
      ...
      {isNearAltar && (isLocked || isMobile) && !prayerOpen && (
      ...
      <div 
        className="absolute bottom-6 right-6 z-40 flex flex-col items-end gap-1.5"
        onClick={(e) => {
          e.stopPropagation();
